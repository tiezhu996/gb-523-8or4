...
- `/planner`：选择负载创建草稿，把负载固定到指定机柜（或取消固定），执行确定性候选布局，查看逐机柜结果、热传播、评分和约束证据。
...
草稿机柜固定：规划页可把任意草稿负载固定到指定机柜，也可取消固定（空固定列表即清空）。固定关系随草稿持久化并带版本号，刷新页面后可回读；取消固定只影响固定关系本身，不改动其他结果。固定项仍受机柜功率、气流、U 位、热区冷量/回风温度和冗余组跨区隔离限制：评估时固定项按负载 ID 顺序优先占用指定机柜，任一项不满足则整体评估失败（HTTP 409 `PIN_CONFLICT`，错误体 `details` 返回具体机柜、热区或负载冲突），草稿与固定关系原样保留；全部满足时固定项优先落位，其余负载仍按确定性算法生成。固定设置只做结构校验（负载属于本草稿、机柜存在、草稿状态与版本），容量可行性统一在评估时判定，因此不可行的固定会保留在草稿上等待修正。
...
| `PUT /api/v1/scenarios/:id/pins` | 设置或取消草稿负载的机柜固定（乐观锁版本） |
| `POST /api/v1/scenarios/:id/evaluate` | 版本校验后执行规划（固定冲突返回 409 `PIN_CONFLICT` 与 details） |
